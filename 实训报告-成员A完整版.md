...
（3）多模式控制架构：小车支持蓝牙遥控、自动避障和自动跟随三种工作模式，通过全局 work_mode 变量进行模式切换和管理。自动避障采用三级递进策略（红外防撞 → 超声波空间扫描 → 试探转向/掉头），跟随模式通过比例速度控制保持与前方目标的固定距离。
...
| 自动跟随 | 超声波测距锁定目标，比例速度控制保持 15cm 距离 | 比例控制 + 确认机制 |
...
**实现方法**：跟随模式通过超声波实时测距，维持与前方目标物体的固定距离（目标 15cm，死区 ±5cm，即 10~20cm）。采用二状态机，在主循环 Follow_Task() 中周期调用（~100ms/次）。

状态0（搜索目标）：舵机三角度扫描（60°、120°、90°），每角度延时 100ms 后测距。取三个方向中最短有效距离，< 80cm 则锁定目标切换到状态1。连续搜索 3 次未找到则原地左转 400ms 换方向。

状态1（跟随阶段）：
1. 丢失二次确认：连续 2 周期无有效回波才确认丢失。确认前扫描 ±20°（70° 和 110°），发现目标在侧面则自动转向追踪。

2. 比例速度控制（基于全局 speed）：距离偏差 error = d - 15。目标速度 = speed + |error| × 12。前进上限 speed×1.5，后退上限 speed，后退基速 speed×0.75。

3. 方向换向缓冲：前进↔后退间停一个周期再启动。
...
{
    static u8 init = 0, state = 0, tries = 0, lost_cnt = 0;
    static s16 hold_speed = 0;
    float d;
    s16 error, target_speed;

    // 模式切换检测
    { static u8 last_mode = 0xFF;
      if (last_mode != work_mode) { init = 0; last_mode = work_mode; } }
    if (!init) { Servo_SetAngle(90); init = 1; return; }

    // 状态0: 搜索
    if (state == 0) {
        Servo_SetAngle(60);  delay_ms(100); float dl = SR04_GetDistance();
        Servo_SetAngle(120); delay_ms(100); float dr = SR04_GetDistance();
        Servo_SetAngle(90);  delay_ms(100); float dc = SR04_GetDistance();
        float best = 999;
        if (dl>0 && dl<best) best=dl;
        if (dr>0 && dr<best) best=dr;
        if (dc>0 && dc<best) best=dc;
        if (best < 80) { state=1; lost_cnt=0; hold_speed=0;
                         UI_SetMode(FACE_NORMAL); }
        else { tries++; Motor_Stop();
               if (tries>=3) { tries=0; Motor_Left(speed*5/8);
                               delay_ms(400); Motor_Stop(); }
               UI_SetMode(FACE_SAD); }
        return;
    }
    ...
    if (d <= 0 || d > 80) {
        ...
        if (lost_cnt >= 2) {
            ...
            if ((dl2>0 && dl2<80) || (dr2>0 && dr2<80)) {
                if (dl2 < dr2) Motor_Left(speed*7/10);
                else Motor_Right(speed*7/10);
                delay_ms(120); Motor_Stop();
                lost_cnt=0; hold_speed=0; UI_SetMode(FACE_NORMAL);
                return;
            }
            state=0; tries=0; lost_cnt=0; hold_speed=0;
            Motor_Stop(); UI_SetMode(FACE_SAD);
        } else { Motor_Stop(); }
        return;
    }
    lost_cnt = 0;
    ...
    error = (s16)(d - 15);
    if (error > 5) {
        target_speed = speed + (error-5)*12;
        if (target_speed > speed*3/2) target_speed = speed*3/2;
        if (hold_speed < -50) { Motor_Stop(); hold_speed=0; return; }
        Motor_Forward((uint16_t)target_speed);
        hold_speed = target_speed; UI_SetMode(FACE_NORMAL);
    } else if (error < -5) {
        target_speed = speed*3/4 + (-error-5)*12;
        if (target_speed > speed) target_speed = speed;
        if (hold_speed > 50) { Motor_Stop(); hold_speed=0; return; }
        Motor_Backward((uint16_t)target_speed);
        hold_speed = -target_speed; UI_SetMode(FACE_ALERT);
    } else {
        Motor_Stop(); hold_speed = 0; UI_SetMode(FACE_NORMAL);
    }
}
```
...
（2）算法子系统：设计并实现了空间扫描最长连续可通行段决策算法、四级递进避障状态机和跟随比例速度控制逻辑（含丢失二次确认、小角度重捕获、方向换向缓冲等优化）。

（3）系统整合：负责主程序的架构设计和时序调度，将团队各成员开发的 30 余个模块统一整合，确保模块间接口正确、时序合理、中断优先级不冲突。

（4）团队协调：制定模块接口规范，分配任务并跟踪进度。

通过本项目，我深刻理解了 MQTT 协议的报文结构和变长编码机制、有限状态机在嵌入式实时控制中的应用、多传感器数据融合的决策算法设计，以及嵌入式系统的资源管理和时序调度。

### 5.2 不足与展望

（1）MQTT 安全性：当前使用非加密 MQTT（端口 1883），设备凭证以明文存储。后续可考虑使用 TLS/SSL 加密（端口 8883）。

（2）OTA 升级：当前固件烧录依赖有线 SWD 接口，后续可利用 ESP8266 实现远程固件更新。

（3）避障算法优化：当前仅在二维平面做扇区扫描，后续可引入 IMU 陀螺仪实现更精准的 3D 环境建模。

（4）云平台深度集成：后续可利用规则引擎实现数据异常告警，将温湿度数据存入云数据库。

（5）跟随模式增强：当前依赖单颗超声波定点测距，后续可加入 PID 控制或融合多传感器扩大跟踪视场角。
